...
impl CoreEngine {
    ...
    pub fn process_key(&self, keycode: u32, mask: u32) -> Option<ImeState> {
        self.0.lock().unwrap().as_ref().map(|e| e.process_key(keycode, mask))
    }

    pub fn select_candidate(&self, index: usize) -> Option<ImeState> {
        self.0.lock().unwrap().as_ref().map(|e| e.select_candidate(index))
    }

    pub fn change_page(&self, backward: bool) -> Option<ImeState> {
        self.0.lock().unwrap().as_ref().map(|e| e.change_page(backward))
    }
